 ...
 * is not required to be atomic in {@link javax.cache.Cache} and is therefore not required to
...
public interface CacheWriter<K, V> {
   ...
   * {@link javax.cache.Cache#putAll(java.util.Map)} also has undefined ordering.
   * <p>
   ...
   * {@link javax.cache.Cache#removeAll(java.util.Set)} also has undefined ordering.
   * <p>
   * If this operation fails (by throwing an exception) after a partial success,
   * the writer must remove any successfully written entries from the entries
   * collection so that the caching implementation knows what succeeded and can
