 ...
 */
public class UserUniqueIDResolverCache {

    private static Log log = LogFactory.getLog(UserIdResolverCache.class);
    private static UserUniqueIDResolverCache userUniqueIDResolverCache = new UserUniqueIDResolverCache();
    private static final String USER_UNIQUE_ID_RESOLVER_CACHE_MANAGER = "USER_UNIQUE_ID_RESOLVER_CACHE_MANAGER";

    private UserUniqueIDResolverCache() {
    }
     ...
     */
    public synchronized static UserUniqueIDResolverCache getInstance() {

        return userUniqueIDResolverCache;
    }

    private Cache<String, String> UserUniqueIdResolverCache(String cacheName) {

        Cache<String, String> cache;
        CacheManager cacheManager = Caching.getCacheManagerFactory().getCacheManager(
                USER_UNIQUE_ID_RESOLVER_CACHE_MANAGER);
    ...
    public void addToCache(String key, String entry, String cacheName, int tenantId) {
        ...
        try {
            ...
            Cache<String, String> cache = UserUniqueIdResolverCache(cacheName);
            if (cache != null && !cache.containsKey(key)) {
                cache.put(key, entry);
                if (log.isDebugEnabled()) {
                    log.debug("Cache: " + cacheName + " which is under " + USER_UNIQUE_ID_RESOLVER_CACHE_MANAGER + "," +
