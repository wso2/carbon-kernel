 ...
 * org.wso2.carbon.user.core.tenant.FileSystemRealmConfigBuilder is used as the tenant manager
...
public class FileSystemRealmConfigBuilder implements MultiTenantRealmConfigBuilder {
    ...
    public RealmConfiguration getRealmConfigForTenantToPersist(RealmConfiguration bootStrapConfig,
        ...
        try {
            ...
            String errorMessage =
                    "Error while building tenant specific realm configuration to be persisted for tenant id : " +
                    tenantId;
            if (logger.isDebugEnabled()) {
                logger.debug(errorMessage, e);
            }
