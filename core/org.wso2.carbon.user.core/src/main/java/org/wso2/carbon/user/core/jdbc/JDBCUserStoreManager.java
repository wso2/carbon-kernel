...
//
    ...
    public Map<String, String> getUserPropertyValues(String userName, String[] propertyNames,
        ...
        try {
            ...
            sqlStmt = replaceUserAttrColumnWithUnicodeColumn(sqlStmt);
            prepStmt = dbConnection.prepareStatement(sqlStmt);
    ...
    public void addProperty(Connection dbConnection, String userName, String propertyName,
        ...
        try {
            ...
            String sqlStmt = realmConfig.getUserStoreProperty(JDBCRealmConstants.ADD_USER_PROPERTY
                    + "-" + type);
            if (sqlStmt == null) {
                if (shouldWriteToBothUserAttColumns()) {
            ...
            } else if (UserCoreConstants.MSSQL_TYPE.equals(type) && shouldWriteToBothUserAttColumns()) {
            ...
            if (sqlStmt.contains(UserCoreConstants.UM_TENANT_COLUMN)) {
                ...
                } else if (shouldWriteToBothUserAttColumns()) {
    ...
    protected SqlBuilder getQueryString(boolean isGroupFiltering, boolean isUsernameFiltering, boolean
        ...
        if (isGroupFiltering && isClaimFiltering) {
            ...
            sqlStatement = new StringBuilder(
                    replaceUserAttrColumnWithUnicodeColumn(sqlStatement.toString()));
    ...
    protected String replaceUserAttrColumnWithUnicodeColumn(String sqlStmt) {

        if (StringUtils.isNotBlank(sqlStmt) && STATE_UNICODE.equalsIgnoreCase(getUserAttributeMigrationState())) {
            return sqlStmt.replace("UM_ATTR_VALUE", "UM_ATTR_VALUE_UNICODE");
        }
        return sqlStmt;
    }

    protected boolean shouldWriteToBothUserAttColumns() {

        return STATE_INTERMEDIATE.equalsIgnoreCase(getUserAttributeMigrationState())
                || STATE_HYBRID.equalsIgnoreCase(getUserAttributeMigrationState());
    }

    protected boolean shouldReadFromBothUserAttrColumns() {

        return STATE_HYBRID.equalsIgnoreCase(getUserAttributeMigrationState());
    }

    private String getUserAttributeMigrationState() {

        return realmConfig.getUserStoreProperty(JDBCRealmConstants.USER_ATTRIBUTE_MIGRATION_STATE);
    }
