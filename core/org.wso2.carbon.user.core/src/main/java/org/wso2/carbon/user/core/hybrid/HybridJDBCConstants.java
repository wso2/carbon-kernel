...
public class HybridJDBCConstants {
    ...
    public static final String GET_ROLES_PAGINATED_DB2 = "SELECT UM_ROLE_NAME FROM(SELECT ROW_NUMBER() OVER (ORDER BY" +
            " UM_ROLE_NAME) AS rn, U.*  FROM UM_HYBRID_ROLE AS U) " +
            "WHERE UM_ROLE_NAME LIKE ? AND UM_TENANT_ID=? AND rn BETWEEN ? AND ?";
    ...
    public static final String GET_INTERNAL_ROLES_PAGINATED_DB2 =
            "SELECT UM_ROLE_NAME FROM (SELECT ROW_NUMBER() OVER (ORDER BY UM_ROLE_NAME) AS rn, p.*  FROM " +
                    "(SELECT DISTINCT UM_ROLE_NAME  FROM UM_HYBRID_ROLE U WHERE U.UM_TENANT_ID = ? AND " +
                    "U.UM_ROLE_NAME LIKE ? AND NOT(UM_ROLE_NAME LIKE ?)) AS p) WHERE rn BETWEEN ? AND ?";
