...
//        setProperty("RolenameJavaRegEx","^[^~!#$;%^*+={}\\\\|\\\\\\\\&lt;&gt;,\\\'\\\"]{3,30}$","A regular expression to validate role names");
        ...
        setProperty(JDBCRealmConstants.VALIDATION_QUERY, "validationQuery for the database", "", "validationQuery is " +
                "the SQL query that will be used to validate connections. This query MUST be an SQL SELECT statement " +
                "that returns at least one row");
        setProperty(JDBCRealmConstants.VALIDATION_INTERVAL, "Validation Interval(time in milliseconds)", "", "Used to" +
