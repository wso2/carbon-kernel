...
public final class JDBCRealmConstants {
    ...
    public static final String GET_ROLE_LIST_PAGINATED_SQL_DB2 =
            "SELECT UM_ROLE_NAME FROM (SELECT ROW_NUMBER() OVER (ORDER BY " +
                    "UM_ROLE_NAME) AS rn, p.*  FROM (SELECT DISTINCT UM_ROLE_NAME  FROM UM_ROLE U WHERE U.UM_ROLE_NAME LIKE ? " +
                    "AND U.UM_TENANT_ID = ?) AS p) WHERE rn BETWEEN ? AND ?";
    public static final String GET_ROLE_LIST_PAGINATED_SQL_MSSQL = "SELECT UM_ROLE_NAME FROM UM_ROLE WHERE " +
            "UM_ROLE_NAME LIKE ? AND UM_TENANT_ID= ? AND UM_SHARED_ROLE ='0' ORDER BY UM_ROLE_NAME OFFSET ? ROW FETCH NEXT ? ROW ONLY";
