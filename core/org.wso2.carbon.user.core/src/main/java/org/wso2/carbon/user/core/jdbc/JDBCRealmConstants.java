...
public final class JDBCRealmConstants {
    ...
    public static final String UPDATE_USER_PROPERTY_WITH_ID_SQL = "UPDATE UM_USER_ATTRIBUTE SET UM_ATTR_VALUE=? WHERE "
            + "UM_ATTR_NAME=? AND UM_PROFILE_ID=? AND UM_USER_ID=(SELECT UM_ID FROM UM_USER WHERE UM_USER_ID=? AND "
            + "UM_TENANT_ID=?) AND UM_TENANT_ID=?";
    public static final String SELECT_USER_PROPERTIES_WITH_ID_SQL = "SELECT * FROM UM_USER_ATTRIBUTE WITH (UPDLOCK) WHERE UM_USER_ID=(SELECT UM_ID FROM UM_USER WHERE UM_USER_ID=? AND UM_TENANT_ID=?) AND UM_TENANT_ID=? ORDER BY UM_ID;";
