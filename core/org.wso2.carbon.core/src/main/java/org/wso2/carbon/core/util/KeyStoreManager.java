/*
*  Copyright (c) 2005-2024, WSO2 LLC. (https://www.wso2.com).
*
...
public class KeyStoreManager {
    ...
    private CachedKeyStore primaryKeyStore = null;
    ...
    private ConcurrentHashMap<String, CachedKeyStore> customKeyStores = null;
    ...
    private static final String ERROR_PRIMARY_KEY_STORE = "Permission denied for accessing primary key store. " +
            "The primary key store is available only for the super tenant.";
    private static final String ERROR_KEYSTORE_NAME = "Key store name is null or empty.";
    ...
    private KeyStoreManager(int tenantId, ServerConfigurationService serverConfigService,
        ...
        customKeyStores = new ConcurrentHashMap<String, CachedKeyStore>();
    ...
    /**
     * Get the key store object for the given key store name.
     * The key store is cached after the first load.
     *
     *
     * @param keyStoreName  Key store name.
     * @return KeyStore object.
     * @throws Exception    If there is an error when retriving the given keystore.
     */
    public CachedKeyStore getCachedKeyStore(String keyStoreName) throws CarbonException {

        if (StringUtils.isEmpty(keyStoreName)) {
            throw new SecurityException(ERROR_KEYSTORE_NAME);
        }
    ...
    public String getKeyStorePassword(String keyStoreName) throws Exception {
        ...
        String path = RegistryResources.SecurityManagement.KEY_STORES + RegistryConstants.PATH_SEPARATOR + keyStoreName;
    ...
    public void updateKeyStore(String name, KeyStore keyStore) throws Exception {
        ...
        String path = RegistryResources.SecurityManagement.KEY_STORES + RegistryConstants.PATH_SEPARATOR + name;
     ...
     *                   than tenant 0
     * @deprecated Use {@link #getCachedPrimaryKeyStore()} instead.
     */
    @Deprecated
    ...
    /**
     * Load the primary key store, this is allowed only for the super-tenant.
     * The primary key store is cached after the first load.
     *
     * @return Primary key store object/
     * @throws CarbonException Carbon Exception for tenants other than the super-tenant.
     */
    public CachedKeyStore getCachedPrimaryKeyStore() throws CarbonException {
        ...
        if (tenantId == MultitenantConstants.SUPER_TENANT_ID) {
            if (primaryKeyStore == null) {
                ...
                String password = config
                        .getFirstProperty(RegistryResources.SecurityManagement.SERVER_PRIMARY_KEYSTORE_PASSWORD);
                String keystoreType = config
                        .getFirstProperty(RegistryResources.SecurityManagement.SERVER_PRIMARY_KEYSTORE_TYPE);
                KeyStore store;
                try {
                    store = KeyStore.getInstance(keystoreType);
                } catch (KeyStoreException e) {
                    throw new CarbonException("Error initializing KeyStore instance.", e);
                }
                try (FileInputStream in = new FileInputStream(file)) {
                    store.load(in, password.toCharArray());
                    primaryKeyStore = new CachedKeyStore(store);
                } catch (IOException | NoSuchAlgorithmException | CertificateException e) {
                    throw new CarbonException("Error loading the key store.", e);
                }
            }
     ...
     * @throws CarbonException CarbonException if failed to retrieve the given keystore from registry.
     */
    private CachedKeyStore getTenantKeyStore(String keyStoreName) throws CarbonException {

        if (log.isDebugEnabled()) {
            log.debug("Loading tenant key store : " + keyStoreName);
        }
        if (isCachedTenantKeyStoreValid(keyStoreName)) {
            return tenantKeyStores.get(keyStoreName).getCachedKeyStore();
        }
        ...
        try {
            ...
            throw new CarbonException("Error initializing KeyStore instance.", e);
                 ...
                 CryptoException | org.wso2.carbon.registry.api.RegistryException e) {
            throw new CarbonException("Error while loading tenant key store: " + keyStoreName, e);
        }
    }
     ...
     */
    private CachedKeyStore getCustomKeyStore(String keyStoreName) throws CarbonException {
        ...
        CachedKeyStore keyStore = new CachedKeyStore(loadKeyStoreFromFileSystem(location, password, type));
     ...
     */
    private PrivateKey getCustomKeyStorePrivateKey(String keyStoreName) throws CarbonException {
     ...
     */
    private Certificate getCustomKeyStoreCertificate(String keyStoreName) throws CarbonException {
