...
public class KeyStoreBasedExternalCryptoProvider implements ExternalCryptoProvider {
    ...
    public HybridEncryptionOutput hybridEncrypt(HybridEncryptionInput hybridEncryptionInput, String symmetricAlgorithm, String asymmetricAlgorithm,
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Hybrid encrypt initiated with '%s' symmetric algorithm, '%s' asymmetric " +
                            "algorithm using '%s' JCE provider related to '%s' crypto context.", symmetricAlgorithm,
                    asymmetricAlgorithm, jceSecurityProvider, cryptoContext));
        }
        ...
        try {
            ...
            byte[] encryptedData = symmetricEncryptData(hybridEncryptionInput, jceSecurityProvider,
                    symmetricAlgorithm, symmetricKey, algoParams);
            ...
            if (log.isDebugEnabled()) {
                log.debug(String.format("Secret key value was successfully encrypted with %s asymmetric algorithm " +
                        "with public certificate : %s", asymmetricAlgorithm, certificateInfo));
            }
    ...
    public byte[] hybridDecrypt(HybridEncryptionOutput hybridEncryptionOutput, String symmetricAlgorithm, String asymmetricAlgorithm,
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Symmetric key value was successfully decrypted with %s asymmetric algorithm " +
                    "and '%s' private key", asymmetricAlgorithm, privateKeyInfo.getKeyAlias()));
        }
        ...
        try {
            byte[] decryptedData = symmetricDecryptData(hybridEncryptionOutput, jceSecurityProvider,
    ...
    protected byte[] symmetricEncryptData(HybridEncryptionInput hybridEncryptionInput, String jceSecurityProvider,
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Plain data was successfully encrypted with %s symmetric algorithm and %s " +
                    "secret key.", symmetricAlgorithm, symmetricKey.getAlgorithm()));
        }
    ...
    private byte[] symmetricDecryptData(HybridEncryptionOutput hybridEncryptionOutput, String jceSecurityProvider,
        ...
        if ((hybridEncryptionOutput.getAuthData() != null) && (hybridEncryptionOutput.getAuthTag() != null)) {
            ...
            return cipher.doFinal(concatByteArrays(new byte[][]{hybridEncryptionOutput.getCipherData(),
                    hybridEncryptionOutput.getAuthTag()}));
        } else {
    ...
    private String[] resolveSymmetricAlgorithm(String symmetricAlgorithm) throws CryptoException {
        ...
        if (symmetricAlgorithm.contains(AlgorithmConstants.ENC_METHOD_AES)) {
            ...
            String errorMessage = String.format("'%s' symmetric algorithm is not supported for hybrid " +
                    "encryption/decryption.", symmetricAlgorithm);
            throw new CryptoException(errorMessage);
        }
