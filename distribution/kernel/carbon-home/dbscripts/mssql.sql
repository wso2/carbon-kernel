...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[UM_ORG_GOVERNANCE_POLICY]') AND TYPE IN (N'U'))
CREATE TABLE UM_ORG_GOVERNANCE_POLICY (
    UM_ID INTEGER IDENTITY(1, 1) NOT NULL,
    ...
    CONSTRAINT FK_ORG_GOV_POLICY_GOVERNING_ORG FOREIGN KEY (UM_GOVERNING_ORG_ID)
        REFERENCES UM_ORG (UM_ID),
    UNIQUE (UM_GOVERNING_ORG_ID, UM_RESOURCE_TYPE, UM_CAPABILITY)
);

-- create table UM_ORG_GOVERNANCE_POLICY_SELECTED_ORGS
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[UM_ORG_GOVERNANCE_POLICY_SELECTED_ORGS]') AND TYPE IN (N'U'))
CREATE TABLE UM_ORG_GOVERNANCE_POLICY_SELECTED_ORGS (
    UM_ID INTEGER IDENTITY(1, 1) NOT NULL,
...
CREATE INDEX IDX_ORG_GOV_SEL_ORG ON UM_ORG_GOVERNANCE_POLICY_SELECTED_ORGS (UM_SELECTED_ORG_ID);
...
CREATE TRIGGER UM_ORG_DELETE_TRIGGER ON UM_ORG INSTEAD OF DELETE AS BEGIN DELETE FROM UM_ORG_DISCOVERY WHERE UM_ROOT_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_DISCOVERY WHERE UM_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_HIERARCHY WHERE UM_PARENT_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_HIERARCHY WHERE UM_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_RESOURCE_SHARING_POLICY WHERE UM_INITIATING_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_RESOURCE_SHARING_POLICY WHERE UM_POLICY_HOLDING_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_GOVERNANCE_POLICY WHERE UM_GOVERNING_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_GOVERNANCE_POLICY_SELECTED_ORGS WHERE UM_SELECTED_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG WHERE UM_PARENT_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG WHERE UM_ID IN (SELECT UM_ID FROM deleted) END;
