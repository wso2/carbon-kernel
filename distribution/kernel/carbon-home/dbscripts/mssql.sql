...
CREATE TABLE UM_HYBRID_USER_ROLE_RESTRICTED_EDIT_PERMISSIONS (
            ...
            PRIMARY KEY (UM_ID),
            FOREIGN KEY (UM_HYBRID_USER_ROLE_ID, UM_HYBRID_USER_ROLE_TENANT_ID) REFERENCES UM_HYBRID_USER_ROLE(UM_ID, UM_TENANT_ID) ON DELETE CASCADE ON UPDATE RESTRICT
);
...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[UM_SHARED_USER_RESTRICTED_EDIT_PERMISSIONS]') AND TYPE IN (N'U'))
CREATE TABLE UM_SHARED_USER_RESTRICTED_EDIT_PERMISSIONS (
    UM_ID INTEGER IDENTITY(1, 1) NOT NULL,
