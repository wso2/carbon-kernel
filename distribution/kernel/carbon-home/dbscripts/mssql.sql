...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[UM_ORG_GOVERNANCE_ORG_SELECTED]') AND TYPE IN (N'U'))
CREATE TABLE UM_ORG_GOVERNANCE_ORG_SELECTED (
    UM_ID INTEGER IDENTITY(1, 1) NOT NULL,
...
CREATE TRIGGER UM_ORG_DELETE_TRIGGER ON UM_ORG INSTEAD OF DELETE AS BEGIN DELETE FROM UM_ORG_DISCOVERY WHERE UM_ROOT_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_DISCOVERY WHERE UM_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_HIERARCHY WHERE UM_PARENT_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_HIERARCHY WHERE UM_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_RESOURCE_SHARING_POLICY WHERE UM_INITIATING_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_RESOURCE_SHARING_POLICY WHERE UM_POLICY_HOLDING_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_GOVERNANCE_POLICY WHERE UM_GOVERNING_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG_GOVERNANCE_ORG_SELECTED WHERE UM_TARGET_ORG_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG WHERE UM_PARENT_ID IN (SELECT UM_ID FROM deleted) DELETE FROM UM_ORG WHERE UM_ID IN (SELECT UM_ID FROM deleted) END;
