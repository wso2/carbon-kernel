...
CREATE TABLE UM_SHARED_USER_RESTRICTED_EDIT_PERMISSIONS (
    UM_ID DECIMAL(31,0) NOT NULL,
    UM_USER_ASSOCIATION_ID DECIMAL(31,0) NOT NULL,
    UM_EDIT_OPERATION VARCHAR(255) NOT NULL,
    UM_PERMITTED_ORG_ID VARCHAR(36) NOT NULL,
    PRIMARY KEY (UM_ID),
    FOREIGN KEY (UM_USER_ASSOCIATION_ID)
        REFERENCES UM_ORG_USER_ASSOCIATION(UM_ID)
        ON DELETE CASCADE
        ON UPDATE CASCADE
)/

CREATE SEQUENCE UM_SHARED_USER_RESTRICTED_EDIT_PERMISSIONS_SEQ AS DECIMAL(27,0)
    INCREMENT BY 1
    START WITH 1
    NO CACHE/

CREATE TRIGGER UM_SHARED_USER_RESTRICTED_EDIT_PERMISSIONS_TRG
    NO CASCADE BEFORE INSERT
    ...
    MODE DB2SQL
BEGIN ATOMIC
    SET (NEW.UM_ID) = (NEXTVAL FOR UM_SHARED_USER_RESTRICTED_EDIT_PERMISSIONS_SEQ);
END/
...
CREATE TABLE UM_RESOURCE_SHARING_POLICY (
    ...
    PRIMARY KEY (UM_ID),
    FOREIGN KEY (UM_INITIATING_ORG_ID)
        REFERENCES UM_ORG(UM_ID)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    FOREIGN KEY (UM_POLICY_HOLDING_ORG_ID)
        REFERENCES UM_ORG(UM_ID)
        ON DELETE CASCADE
        ON UPDATE CASCADE
)/
...
CREATE TRIGGER UM_RESOURCE_SHARING_POLICY_TRG
    NO CASCADE BEFORE INSERT
    ...
    MODE DB2SQL
BEGIN ATOMIC
    SET (NEW.UM_ID) = (NEXTVAL FOR UM_RESOURCE_SHARING_POLICY_SEQ);
END/
...
CREATE TRIGGER UM_SHARED_RESOURCE_ATTRIBUTES_TRG
    NO CASCADE BEFORE INSERT
    ...
    MODE DB2SQL
BEGIN ATOMIC
    SET (NEW.UM_ID) = (NEXTVAL FOR UM_SHARED_RESOURCE_ATTRIBUTES_SEQ);
END/
