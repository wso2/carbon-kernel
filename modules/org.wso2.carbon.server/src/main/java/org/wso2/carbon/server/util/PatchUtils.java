...
public class PatchUtils {
    ...
    /**
     * This will copy only the newly added patch files to the plugins directory
     * @param servicepackDir servicepack directory
     * @param patchesDir patch directory
     * @param mostRecentPatch most recent patch from the newly added patch
    ...
    public static void copyNewPatches(File servicepackDir, File patchesDir, String mostRecentPatch, File pluginsDir) throws IOException {
        File[] files = patchesDir.listFiles(PatchUtils.getPatchFileNameFilter());
        Arrays.sort(files);
        File patchDirLogFile = new File(PatchUtils.getMetaDirectory() , LauncherConstants.PRE_PATCHED_DIR_FILE);
        BufferedWriter bufWriter = new BufferedWriter(new FileWriter(patchDirLogFile));
        List<String> patchApplyingOrder = getPatchApplyingOrder(servicepackDir, patchesDir);

        // since we only apply set of patches we need to maintain the state of the PRE_PATCHED_DIR_FILE
        for (String appliedPatch : patchApplyingOrder) {
            if (mostRecentPatch.equalsIgnoreCase(appliedPatch)) {
                break;
            } else {
                try {
                    // write applying patches directory names to LauncherConstants.PRE_PATCHED_DIR_FILE
                    ...
                    patchLog.error("Error occurred while updating " + appliedPatch +
                            " directory name to " + patchDirLogFile.getName());
                }
            }
        }
        ...
        try {
                for (File file : files) {
                    if (file.isDirectory()) {
                        // if patch file is in the new patch list we apply the patch otherwise skip
                        if (file.getName().compareTo(mostRecentPatch) >= 0) {
                            // verify bundleFileName before copying the files to plugins
                            File[] patchFiles = file.listFiles();
                            patchLog.info("Applying - " + file.getName());
                            for (File patch : patchFiles) {
    ...
    public static Map<String, JarInfo> getJarsInAppliedServicepackAndPatches(File servicepackDir, File parentPatchDir) throws Exception {
        // this map contains all latest patched jars
        Map<String, JarInfo> currentlyPatchedJars = new HashMap<String, JarInfo>();
        if (previousPatchDirNames.size() > 0) {
            if (previousPatchDirNames.get(0).startsWith("s")) {
                // first lets take jars in servicepack directories
                File[] servicepackList = servicepackDir.listFiles(PatchUtils.getPatchFileNameFilter());
                if (servicepackList != null && servicepackList.length > 0) {
                    // Sorting servicepack folders.
                    Arrays.sort(servicepackList);
                    for (File servicepack : servicepackList) {
                        if (servicepack.getName().equalsIgnoreCase(previousPatchDirNames.get(0))) {
                            if (servicepack.isDirectory()) {
                                // get latest jar file list of a servicepack
                                File libDir = FileUtils.getFile(servicepack, LauncherConstants.SERVICEPACK_LIB_DIR);
                                collectLatestJars(libDir, currentlyPatchedJars);
                            }
                            break;
                        }
                    }
            ...
            // then take jars in patch directories
            getJarsInPreviouslyAppliedPatches(parentPatchDir, servicepackPatchedList, currentlyPatchedJars, previousPatchDirNames);
        }
        // check any servicepacks has applied
        return currentlyPatchedJars;
    }

    /**
     * get the list of jars in previously applied patches-xxx directories
     *
     * @param parentPatchDir     folder which contains the patches.
     * @param servicepackPatches
     * @param latestPatchedJars
     * @param previousPatchDirNames list of the previously applied patches
     */
    private static void getJarsInPreviouslyAppliedPatches(File parentPatchDir, Set<String> servicepackPatches, Map<String, JarInfo> latestPatchedJars, List<String> previousPatchDirNames) throws Exception {
        ...
        if (patchList != null) {
            Arrays.sort(patchList);
            for (File file : patchList) {
                if (file.isDirectory() && !servicepackPatches.contains(file.getName()) && previousPatchDirNames.contains(file.getName())) {
                    // if it's the patch0000 directory ignore it
                    // patch0000 directory is backup directory .
                    if (file.getName().equals(LauncherConstants.BUNDLE_BACKUP_DIR)) {
                        // do not need to check LauncherConstants.BUNDLE_BACKUP_DIR for latest jars
    ...
    public static void checkMD5Checksum(Map<String, JarInfo> latestPatchedJar, File plugins, boolean applyPatches) throws Exception {
        ...
        try{
            for (Map.Entry<String, JarInfo> entry : latestPatchedJar.entrySet()) {
                ...
                if (file == null) {
                    warningList.add(entry.getKey() + "(MD5: " + md5OfPatchedJar + ") has been patched with " +
                            PatchUtils.getPatchedDirName(entry.getValue().getPath()) + ", but not applied");
                } else {
            ...
            if (warningList.size() > 0) {
                ...
                patchLog.info("Patch verification successfully completed without encountering any issues");
                if (applyPatches) {
                    consoleLog.info("Patch verification successfully completed without encountering any issues.");
                }
            }
        }finally {
    ...
    public static File getPatchesLogsFile() {
        return new File(Utils.getCarbonRepoPath() + File.separator + "logs" +
                File.separator + LauncherConstants.PATCH_LOG_FILE);
    }
